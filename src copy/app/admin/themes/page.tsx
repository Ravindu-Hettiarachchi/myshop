...
function RadioCard({ v, current, onChange, l, d }: { v: string; current: string; onChange: (x: string) => void; l: string; d: string }) {
    ...
    return (
        <button type="button" onClick={() => onChange(v)}
            className={`text-left px-3 py-2.5 rounded-xl border-2 transition-all ${sel ? 'border-purple-600 bg-purple-50/50' : 'border-gray-100 hover:border-purple-200 bg-gray-50/40'}`}>
            <div className="flex items-center gap-2">
                <div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${sel ? 'border-purple-600' : 'border-gray-300'}`}>
                    {sel && <div className="w-2 h-2 rounded-full bg-purple-600" />}
                </div>
                <span className={`font-semibold text-xs ${sel ? 'text-purple-800' : 'text-gray-800'}`}>{l}</span>
            </div>
            <p className="text-xs text-gray-400 mt-0.5 ml-5 leading-tight">{d}</p>
        </button>
    );
}
...
function ColorField({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
    return (
        <div>
            ...
            <div className="flex items-center gap-2">
                <input type="color" value={value} onChange={e => onChange(e.target.value)} className="w-9 h-9 rounded-lg border-none cursor-pointer flex-shrink-0" />
                <input type="text" value={value} onChange={e => onChange(e.target.value)} className="flex-1 px-2.5 py-2 rounded-lg border border-gray-200 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500" />
            </div>
        </div>
    );
}
...
function Section({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
    return (
        <div className="bg-gray-50/60 rounded-2xl p-5 space-y-4 border border-gray-100">
            <p className="text-sm font-bold text-gray-800 flex items-center gap-2">{icon}{title}</p>
            {children}
        </div>
    );
}
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => onChange(!value)} className="flex-shrink-0">
                {value ? <ToggleRight className="w-7 h-7 text-green-500" /> : <ToggleLeft className="w-7 h-7 text-gray-400" />}
            </button>
        </div>
    );
}
...
export default function AdminThemesPage() {
    ...
    const c = customForm; // shorthand for live preview
    const activeCount = themes.filter(t => t.is_active).length;

    return (
        <div className="p-6 lg:p-8 space-y-8 max-w-6xl mx-auto text-gray-900 font-sans">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">
                        ...
                        <span className="font-semibold text-gray-700">{activeCount} active</span> / {themes.length} total
                    </p>
                </div>
                <button onClick={() => { setShowForm(v => !v); setError(''); }}
                    className="flex items-center gap-2 bg-gray-900 hover:bg-black text-white px-5 py-2.5 rounded-xl text-sm font-bold transition shadow-sm">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-2xl overflow-hidden">
                <button onClick={() => setShowHint(v => !v)}
                    className="w-full flex items-center gap-2 px-5 py-3 text-blue-800 hover:bg-blue-100 transition text-sm font-semibold">
                ...
                {showHint && (
                    <div className="px-5 pb-4 pt-1 border-t border-blue-100">
                        <p className="text-sm text-blue-600 mb-3">
                            These slugs are registered in <code className="bg-blue-100 px-1.5 py-0.5 rounded font-mono text-xs">src/lib/themes.ts</code>.
                        ...
                        <div className="flex flex-wrap gap-2">
                            {registeredSlugs.map(slug => (
                                <button key={slug} onClick={() => { setCodedForm(f => ({ ...f, slug })); setThemeType('coded'); setShowForm(true); }}
                                    className="font-mono text-xs bg-white border border-blue-200 text-blue-700 px-3 py-1 rounded-lg hover:border-blue-400 transition">
                                    {slug}
                                </button>
                            ))}
                        </div>
            ...
            {showForm && (
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 space-y-6">
                    {/* Type Toggle */}
                    <div>
                        <p className="text-sm font-semibold text-gray-700 mb-3">Theme Type</p>
                        <div className="flex gap-3">
                            {[
                                { type: 'coded' as const, Icon: Code2, label: 'Coded Theme', desc: 'Links to a developer .tsx component', color: 'blue' },
                                { type: 'custom' as const, Icon: Wand2, label: 'Custom Builder', desc: 'Design visually — no code needed', color: 'purple' },
                            ].map(({ type, Icon, label, desc, color }) => (
                                <button key={type} type="button" onClick={() => setThemeType(type)}
                                    className={`flex-1 flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition ${themeType === type ? `border-${color}-600 bg-${color}-50` : 'border-gray-100 hover:border-gray-200'}`}>
                                    <Icon className={`w-5 h-5 ${themeType === type ? `text-${color}-600` : 'text-gray-400'}`} />
                                    <div className="text-left">
                                        <p className={`font-bold text-sm ${themeType === type ? `text-${color}-700` : 'text-gray-700'}`}>{label}</p>
                                        <p className="text-xs text-gray-400">{desc}</p>
                                    </div>
                                    {themeType === type && <Check className={`w-4 h-4 text-${color}-600 ml-auto`} />}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleAdd} className="space-y-5">
                        ...
                        {themeType === 'coded' && (
                            <div className="space-y-4">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input value={codedForm.slug} onChange={e => setCodedForm(f => ({ ...f, slug: e.target.value }))}
                                            ...
                                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                                        <p className="text-xs text-gray-400 mt-1">Must match a key in <code>src/lib/themes.ts</code></p>
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center gap-6">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Preview Color</label>
                                        <div className="flex items-center gap-2">
                                            <input type="color" value={codedForm.preview_color} onChange={e => setCodedForm(f => ({ ...f, preview_color: e.target.value }))} className="w-10 h-10 rounded-lg cursor-pointer" />
                                            <input type="text" value={codedForm.preview_color} onChange={e => setCodedForm(f => ({ ...f, preview_color: e.target.value }))} className="w-28 px-3 py-2 rounded-xl border border-gray-200 font-mono text-sm" />
                                        </div>
                                    </div>
                                    <div className="mt-5">
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 mt-1">Slug is auto-generated</p>
                                    </div>
                                    <div>
                                        ...
                                        {Array.from({ length: c.layout_style === 'grid' ? 4 : 3 }).map((_, i) => (
                                            <div key={i} style={{
                                                ...
                                                <div className="p-1.5">
                                                    ...
                                                    <div style={{ background: c.button_style === 'filled' ? c.accent_color : 'transparent', border: c.button_style === 'outline' ? `1.5px solid ${c.accent_color}` : 'none', color: c.button_style === 'filled' ? '#fff' : c.accent_color, borderRadius: c.button_radius === 'pill' ? '999px' : '4px' }} className="h-4 flex items-center justify-center text-white" style2={{ fontSize: '8px', fontWeight: 700 }}>
                                ...
                                {/* ── Section 1: Colors ── */}
                                <Section icon={<Palette className="w-4 h-4 text-purple-500" />} title="Colors">
                                ...
                                <Section icon={<Type className="w-4 h-4 text-purple-500" />} title="Typography">
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-1.5">Body Font</label>
                                            <select value={c.font_family} onChange={e => set('font_family', e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 bg-white">
                                                {FONTS.map(f => <option key={f} value={f}>{f}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-1.5">Heading Font</label>
                                            <select value={c.heading_font} onChange={e => set('heading_font', e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 bg-white">
                                                {FONTS.map(f => <option key={f} value={f}>{f}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-2">Body Size</label>
                                            <div className="grid grid-cols-3 gap-2">
                                                {[{ v: 'sm', l: 'S' }, { v: 'md', l: 'M' }, { v: 'lg', l: 'L' }].map(o => (
                                                    <button key={o.v} type="button" onClick={() => set('body_size', o.v)}
                                                        className={`py-2 rounded-xl border-2 font-bold text-sm transition ${c.body_size === o.v ? 'border-purple-600 bg-purple-50 text-purple-700' : 'border-gray-100 hover:border-purple-200 text-gray-600'}`}>
                                                        {o.l}
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                {/* ── Section 3: Layout ── */}
                                <Section icon={<Layout className="w-4 h-4 text-purple-500" />} title="Layout & Spacing">
                                    <div className="space-y-4">
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Product Grid Layout</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Hero Section</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Spacing Scale</p>
                                ...
                                {/* ── Section 4: Header ── */}
                                <Section icon={<Monitor className="w-4 h-4 text-purple-500" />} title="Header">
                                    <div>
                                        <p className="text-xs font-medium text-gray-600 mb-2">Header Style</p>
                                ...
                                {/* ── Section 5: Cards ── */}
                                <Section icon={<Square className="w-4 h-4 text-purple-500" />} title="Product Cards">
                                    <div className="space-y-4">
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Card Shape</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Image Ratio</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Hover Effect</p>
                                ...
                                {/* ── Section 6: Buttons ── */}
                                <Section icon={<Check className="w-4 h-4 text-purple-500" />} title="Buttons">
                                    <div className="space-y-4">
                                        <div>
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Border Radius</p>
                                ...
                                {/* ── Section 7: Footer ── */}
                                <Section icon={<Footprints className="w-4 h-4 text-purple-500" />} title="Footer">
                                    <div className="space-y-4">
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Footer Layout</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-600 mb-2">Footer Background</p>
                        ...
                        <div className="flex justify-end pt-2">
                            <button type="submit" disabled={saving}
                                className={`px-8 py-2.5 rounded-xl font-bold text-sm text-white transition shadow-sm disabled:opacity-50 ${themeType === 'custom' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-gray-900 hover:bg-black'}`}>
            ...
            <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
                <div className="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
                    <h2 className="font-bold text-gray-900">All Themes</h2>
                ...
                {loading ? (
                    ...
                    <div className="py-20 flex flex-col items-center text-gray-400">
                        ...
                        <p className="text-sm mt-1">Run <code className="bg-gray-100 px-1.5 rounded text-xs">database/themes_migration.sql</code> to seed defaults.</p>
                    </div>
                ) : (
                    <ul className="divide-y divide-gray-100">
                        {themes.map(theme => {
                            ...
                            return (
                                <li key={theme.id} className={`px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition ${theme.is_active ? 'hover:bg-gray-50/50' : 'opacity-55 bg-gray-50/30'}`}>
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-xl flex-shrink-0 border-2 border-white shadow-md" style={{ background: theme.preview_color || theme.accent_color || '#3B82F6' }} />
                                        <div>
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <p className="font-bold text-gray-900">{theme.name}</p>
                                                {isCustom
                                                    ? <span className="text-xs bg-purple-50 text-purple-700 border border-purple-200 px-2 py-0.5 rounded-full flex items-center gap-1"><Wand2 className="w-3 h-3" /> Custom</span>
                                                    : <span className="text-xs bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full flex items-center gap-1"><Code2 className="w-3 h-3" /> Coded</span>
                                                }
                                                {!isCustom && !inRegistry && <span className="text-xs bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-full">⚠ Not in registry</span>}
                                                {!theme.is_active && <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Disabled</span>}
                                            </div>
                                            ...
                                            {isCustom && (
                                                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                                    {[theme.bg_color, theme.text_color, theme.accent_color, theme.secondary_color].map((col, i) => col && (
                                                        <div key={i} className="w-4 h-4 rounded-full border border-gray-200 shadow-sm" style={{ background: col }} />
                                                    ))}
                                                    <span className="text-xs text-gray-400">{theme.font_family} · {theme.layout_style} · {theme.card_style} · {theme.button_style}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <button onClick={() => handleToggle(theme)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition">
                                            {theme.is_active ? <ToggleRight className="w-5 h-5 text-green-500" /> : <ToggleLeft className="w-5 h-5 text-gray-400" />}
                                            {theme.is_active ? 'Active' : 'Inactive'}
                                        </button>
                                        <button onClick={() => handleDelete(theme)} className="p-2.5 rounded-xl border border-gray-200 text-gray-400 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition">
